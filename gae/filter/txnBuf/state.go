...
func (t *txnBufState) getMulti(keys []*datastore.Key, metas datastore.MultiMetaGetter, cb datastore.GetMultiCB, haveLock bool) error {
	...
	err := func() error {
		...
		if len(toGetKeys) > 0 {
			t.bufDS.GetMulti(toGetKeys, nil, func(j int, pm datastore.PropertyMap, err error) {
				impossible(err)
		...
		if len(idxMap) > 0 {
			err := t.parentDS.GetMulti(getKeys, getMetas, func(j int, pm datastore.PropertyMap, err error) {
...
func (t *txnBufState) deleteMulti(keys []*datastore.Key, cb datastore.DeleteMultiCB, haveLock bool) error {
	...
	err := func() error {
		...
		err := t.bufDS.DeleteMulti(keys, func(i int, err error) {
			impossible(err)
...
func (t *txnBufState) fixKeys(keys []*datastore.Key) ([]*datastore.Key, error) {
	...
	err := t.parentDS.AllocateIDs(incompleteKeys, func(i int, key *datastore.Key, err error) {
...
func (t *txnBufState) putMulti(keys []*datastore.Key, vals []datastore.PropertyMap, cb datastore.NewKeyCB, haveLock bool) error {
	...
	err = func() error {
		...
		err := t.bufDS.PutMulti(keys, vals, func(i int, k *datastore.Key, err error) {
			impossible(err)
	...
	return parallel.FanOutIn(func(ch chan<- func() error) {
		if len(toPut) > 0 {
			ch <- func() error {
				...
				err := s.parentDS.PutMulti(toPutKeys, toPut, func(i int, _ *datastore.Key, err error) {
					mErr.Assign(i, err)
				})
				if err == nil {
					err = mErr.Get()
		...
		if len(toDel) > 0 {
			ch <- func() error {
				...
				err := s.parentDS.DeleteMulti(toDel, func(i int, err error) {
					mErr.Assign(i, err)
				})
				if err == nil {
					err = mErr.Get()
...
func (t *txnBufState) commitLocked(s *txnBufState) {
	...
	if len(toPut) > 0 {
		impossible(t.putMulti(toPutKeys, toPut,
			func(_ int, _ *datastore.Key, err error) { impossible(err) }, true))
	}

	if len(toDel) > 0 {
		impossible(t.deleteMulti(toDel,
